...
//    dealoc(element);
  ...
  it('should iterate over an array of objects', function() {
    element = $compile(
      '<ul>' +
        '<li my-repeat="item in items">{{item.name}};</li>' +
      '</ul>')(scope);
  ...
  it('should iterate over an array of primitives', function() {
    element = $compile(
      '<ul>' +
        '<li my-repeat="item in items">{{item}};</li>' +
      '</ul>')(scope);
  ...
  xit('should iterate over object with changing primitive property values', function() {
    // test for issue #933

    element = $compile(
      '<ul>' +
        '<li my-repeat="(key, value) in items">' +
          '{{key}}:{{value}};' +
  ...
  it('should error on wrong parsing of myRepeat', function() {
    expect(function() {
      element = angular.element('<ul><li my-repeat="i dont parse"></li></ul>');
      $compile(element)(scope);
    }).toThrow("Expected myRepeat in form of '_item_ in _collection_' but got 'i dont parse'.");
  });


  it("should throw error when left-hand-side of myRepeat can't be parsed", function() {
    expect(function() {
      element = angular.element('<ul><li my-repeat="i dont parse in foo"></li></ul>');
      $compile(element)(scope);
  ...
  it('should expose iterator offset as $index when iterating over arrays',
    ...
    element = $compile(
      '<ul>' +
        '<li my-repeat="item in items">{{item}}:{{$index}}|</li>' +
      '</ul>')(scope);
  ...
  it('should ignore $ and $$ properties', function() {
    element = $compile('<ul><li my-repeat="i in items">{{i}}|</li></ul>')(scope);
    scope.items = ['a', 'b', 'c'];
  ...
  it('should ignore non-array element properties when iterating over an array', function() {
    element = $compile('<ul><li my-repeat="item in array">{{item}}|</li></ul>')(scope);
    scope.array = ['a', 'b', 'c'];
    scope.array.foo = '23';
